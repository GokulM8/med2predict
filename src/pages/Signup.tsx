...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Account created', description: `Signed in as ${res.user.email}` });
      navigate('/');
    } catch (err) {
      toast({ title: 'Signup failed', description: 'Try a different email or check inputs.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
